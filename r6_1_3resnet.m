...
for j=1:n  % 各画像についての for-loop
    ...
    pool5 = activations(net, reimg, 'pool5'); 
    pool5 = squeeze(pool5);
    pool5 = pool5 / norm(pool5);
    P5(j, :) = pool5;
end
...
[acc_res, all_plabels, all_scores] = r6_five_fold_cv(P5(1:100, :), P5(101:200, :), 'linear');

fprintf('ResNet DCNN (linear SVM) Accuracy: %.2f%%\n', acc_res * 100);

% --- 正解・不正解画像のインデックスを特定 ---
correct_idx = find(true_label == all_plabels); % 判定が合っていたもの
wrong_idx = find(true_label ~= all_plabels); % 判定を間違えたもの

figure('Name', 'ResNet DCNN Classification Results (Correct vs Wrong)');
